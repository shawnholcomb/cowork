...
var helper = {
    ...
    postMessage: (name, image, messageDate, message, cb) => {
        connection.query("INSERT INTO `messages` (`name`, `image`, `messageDate`, `message`) VALUES (?, ?, ?, ?)", [name, image, messageDate, message], (err, res) => {
            if (err) throw err;
            cb(res);
        })
    },
